...
// 1. Remove local player state variables and add global player logic
content = content.replace(
    /const \[currentTime, setCurrentTime\] = useState\(0\);\n    const \[duration, setDuration\] = useState\(0\);\n    const \[volume, setVolume\] = useState\(80\);\n    const \[isMuted, setIsMuted\] = useState\(false\);\n/,
    ""
);

const handleGlobalStateStr = `
    useEffect(() => {
...
// Remove local togglePlay
content = content.replace(/const togglePlay = \(\) => \{[\s\S]*?\}\n    \};\n/, "");

// Remove unused formatTime, handleProgressChange, handleVolumeChange, toggleMute
...
// Remove the <audio> tag and the bottom player bar
content = content.replace(/<audio[\s\S]*?ref={audioRef}[\s\S]*?\/>\n/, "");

// The bottom player starts at {/* Player Bar */} and goes until just before {/* Purchase Modal */}
...
// Update getAlbumCover to include Munchies, Darkside, Lord Knows
content = content.replace(
...
// Add Darkside and Lord Knows to albums array
const addAlbums = `
        {
            name: "Darkside",
...
// Add youtubeTimestamps dictionary
const timestampsStr = `
...
// Rewrite the iframe to add youtube timestamps
content = content.replace(
...
// Modify playTrack to pause global player for YouTube tracks
content = content.replace(
    /const playTrack = \(index: number\) => \{\n        setCurrentTrackIndex\(index\);\n        setIsPlaying\(true\);\n        if \(typeof window !== "undefined"\) \{\n            window\.dispatchEvent\(new CustomEvent\('playMusic', \{ detail: \{ trackIndex: index \} \}\)\);\n        \}\n    \};/,
    `const playTrack = (index: number) => {
...
// Now to overhaul the Featured / New Songs section!
// Let's replace the whole section manually.
...
if (featuredStart !== -1 && albumsGridStart !== -1) {
    const munchiesSection = `{/* Featured / New Songs - Overhauled for Munchies */}
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-12 gap-0">
                                    <div className="lg:col-span-5 p-8 lg:p-12 border-b lg:border-b-0 lg:border-r border-white/10">
                                        ...
                                        {/* Keep SoundCloud Embed at the bottom of the left column */}
