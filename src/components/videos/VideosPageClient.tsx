...
import { FullTapeShelf, TapeTracklist, fullTapeTitle, type FullTape } from "@/components/videos/FullTapeShelf";
...
interface VideosPageClientProps {
    /** Video-only releases, with their tracklists, shelved above the music videos. */
    tapes: FullTape[];
}

export function VideosPageClient({ tapes }: VideosPageClientProps) {
    ...
    // Arriving with a video named in the URL — the old /music/<tape> album
    // links redirect here — put it straight on the screen instead of dropping
    // the visitor at the top of a page of tapes.
    useEffect(() => {
        const requested = new URLSearchParams(window.location.search).get("v");
        if (!requested || !/^[\w-]{6,20}$/.test(requested)) return;
        const tape = tapes.find((t) => t.youtubeId === requested);
        handlePlay(requested, tape ? fullTapeTitle(tape) : undefined);
    }, [handlePlay, tapes]);
    ...
    // The full tapes have their own shelf, so keep them off this one even if
    // someone also adds them through the admin.
    const tapeIds = new Set(tapes.map((t) => t.youtubeId));
    const mainVideos = allVideos.filter(v => {
        const id = v.youtube_id || v.youtubeId;
        return id !== (featuredVideo.youtube_id || featuredVideo.youtubeId) && !tapeIds.has(id ?? "");
    });
    ...
    // Phones play on the theater screen too — a second player under the
    // scene just duplicated the screen that's already there.
    const showInlineFrame = isWatching;
    const tapeOnScreen = tapes.find((t) => t.youtubeId === onScreen.id);
    ...
    return (
        <div className="min-h-screen bg-noir-void text-foreground selection:bg-accent-cyan/30">
            ...
            {/* On a phone the tape shelf is too small to carry tracklists, so the
                one for the tape on screen sits here, under the player. */}
            {tapeOnScreen && (
                <div className="px-6 pb-4 md:hidden">
                    <TapeTracklist
                        key={tapeOnScreen.slug}
                        tape={tapeOnScreen}
                        label={`${tapeOnScreen.name} · ${tapeOnScreen.tracks.length} tracks`}
                        className="border-b"
                    />
                </div>
            )}

            {/* FULL TAPES — the video-only releases, each with its own sleeve */}
            <FullTapeShelf tapes={tapes} nowShowingId={screenVideo?.id} onPlay={handlePlay} />

            {/* MAIN CATALOG */}
            <section className="relative pt-14 pb-24 md:py-24 px-6 z-10">
                <div className="max-w-7xl mx-auto">
                    <div className="flex items-end justify-between mb-8 md:mb-16 border-b border-white/10 pb-4 md:pb-6">
                        <div>
                            ...
                            <h3 className="text-lg sm:text-2xl font-bold text-accent-cyan -mt-6 md:-mt-8 uppercase tracking-widest pl-2">
